...
export default function TVShows() {
    ...
    const { data, loading } = useFetchMainData(`https://api.themoviedb.org/3/discover/tv?&page=${pageNumber}`, true);
    const { data: genres } = useFetchMainData(`https://api.themoviedb.org/3/genre/tv/list`, false);
    ...
    return (
        <div className="container mx-auto px-5 py-[80px]">
            <div className="flex flex-col sm:flex-row justify-between items-center">
                <h1 className="text-[24px] mb-5 sm:mb-0 text-[#fff]">Explore TV Shows</h1>
                <Select
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 2xl:grid-cols-7 gap-5 mt-5">
                {Array.isArray(filteredMovies) && filteredMovies.length > 0 ? (
                    filteredMovies.map((card) => (
                        <Link to={`/tv/${card.id}`} key={`tv-${card.id}`} className="glider-card wahed">
                            ...
                            <div className='mt-5'>
                                <h6 className='text-[#fff] text-[18px] h-[40px]'>{card.name}</h6>
